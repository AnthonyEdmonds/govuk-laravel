...
<table class="govuk-table">
        ...
        <thead class="govuk-table__head">
            <tr class="govuk-table__row">
                @foreach($headings as $heading)
                    <x-govuk::table-cell-header :numeric="$heading[1]">{{ $heading[0] }}</x-govuk::table-cell-header>
                @endforeach
            </tr>
        </thead>
    @endif
    ...
    <tbody class="govuk-table__body">
        @if(count($data) < 1)
            <tr class="govuk-table__row">
                <x-govuk::table-cell-row colspan="{{ count($headings) }}" colour="dark-grey">
                    {!! $emptyMessage !!}
                </x-govuk::table-cell-row>
            </tr>
        @else
            @foreach($data as $row)
                <tr class="govuk-table__row">
